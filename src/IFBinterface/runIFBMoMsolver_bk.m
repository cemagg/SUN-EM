function [ifbmom] = runIFBMoMsolver(Const, Solver_setup, zMatrices, yVectors, xVectors)
    ...
    elseif ((Const.IFBalg == 7)||(Const.IFBalg == 8)||(Const.IFBalg == 9)||(Const.IFBalg == 10)||(Const.IFBalg == 11))
        ...
        if ((Const.IFBalg == 7)||(Const.IFBalg == 9)||(Const.IFBalg == 10)||(Const.IFBalg == 11))
            ...
            if (~Solver_setup.disconnected_domains)
                % We need to call the MBFgenerator to calculate primary MBFs using the generating sub-arrays
                [mbfs] = runMBFgenerator(Const, Solver_setup, zMatrices, yVectors, xVectors);
            end

            for n=1:numArrayElements
                
                % The basis function associated with this particular array element
                domain_n_basis_functions = Solver_setup.rwg_basis_functions_domains{n};

                % Distinguish now between connected and disjoint arrays. The primary MBFs will be
                % generated differently.
                if (Solver_setup.disconnected_domains)
                    % We have alrady calculated the LU decomposition of the domains (all identical)                    
                    ...
                    % Read the MBF from the generating sub-array
                    % Do not summate the MBFs. Rather decide which one to use (based on whether this is an 
                    % edge element in the array or a centre element)                    
                    if (n == 1)
                        % Extract first MBF
                        Ik(domain_n_basis_functions,1) = mbfs.PrimIsol(domain_n_basis_functions,1,n,1);
                    elseif (n== numArrayElements)
                        % Extract Centre MBF
                        Ik(domain_n_basis_functions,1) = mbfs.PrimIsol(domain_n_basis_functions,2,n,1);
                    else
                        % Extract last MBF
                        Ik(domain_n_basis_functions,1) = mbfs.PrimIsol(domain_n_basis_functions,2,n,1);
                    end
            ...
            for k=2:k_iter % Iterations k= 2, ...
                ...
                for n=1:numArrayElements
                    ...
                    if (~Solver_setup.disconnected_domains)
                        ...
                        Const.useACA = 0;        
                        [ZdomB, UacadomB, VacadomB] = ...
                            calcZmn(Const,zMatrices,1,n,n,domain_n_basis_functions,domain_n_basis_functions);
                        [LdomB,UdomB] = lu(ZdomB);
                        % Reset again the ACA usage:
                        Const.useACA = useACAtmp;
                    end
            ...
            for k=2:k_iter % Iterations k= 2, ...
                ...
                for n=1:numArrayElements % To calculate the current on domain p - as noted in [8] (used "n" here just 
                    ...
                    for m=1:numArrayElements
                        ...
                        % Add the nth coupling term for array element n using the (n-1)th coupling                        
