function [Z] = FillZMatrixByEdge_dB(omega,eps_0,mu_0,k,r_c,rho_c_pls,rho_c_mns,quad_pts,sing,dof2edge)
...
for mm = 1:NUM_DOFS
    for nn = 1:NUM_DOFS
        ...
        [MagVecPot,ScalPot] = Potentials(pp_pls,qq_pls,mm,nn,1,k,r_c,dof2edge,quad_pts,sing,eps_0,mu_0,omega);
        Amn_pls_source_pls = MagVecPot;
        Phi_mn_pls_source_pls = -ScalPot;
        [MagVecPot,ScalPot] = Potentials(pp_pls,qq_mns,mm,nn,2,k,r_c,dof2edge,quad_pts,sing,eps_0,mu_0,omega);
        ...
        % Repeat now for m-
        [MagVecPot,ScalPot] = Potentials(pp_mns,qq_pls,mm,nn,1,k,r_c,dof2edge,quad_pts,sing,eps_0,mu_0,omega);
        Amn_mns_source_pls = MagVecPot;
        Phi_mn_mns_source_pls = -ScalPot;
        [MagVecPot,ScalPot] = Potentials(pp_mns,qq_mns,mm,nn,2,k,r_c,dof2edge,quad_pts,sing,eps_0,mu_0,omega);
...
function [MagVecPot,ScalPot] = Potentials(field_pt,source_pt,field_edge, source_edge,source_tri,k,r_c,dof2edge,quad_pts,sing,eps_0,mu_0,omega)
...
if ((field_edge == 6) && (source_edge == 17) && false)    
    fprintf('Debugging element %d,%d\n',field_edge,source_edge);
    fprintf('  Ipq(%d,%d) = %.5f\n',field_pt,source_pt,Ipq);
    fprintf('  length(%d,%d) = %.5f\n',field_pt,source_pt,ELL(dof2edge(source_edge)));
    fprintf('  ScalPot(%d,%d) = %.5f\n',field_pt,source_pt,ScalPot);
    MagVecPot
end

%fprintf('  MagVecPotX(%d,%d) = %.5f + j%.5f\n',field_pt,source_pt,real(MagVecPot(1,1)),imag(MagVecPot(1,1)));
